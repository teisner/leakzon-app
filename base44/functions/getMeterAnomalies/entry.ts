...
Deno.serve(async (req) => {
  try {
    ...
    // Max reading count = the most complete meter sets the baseline
    const meterReadingCounts = {};
    for (const m of allMeters) {
      meterReadingCounts[m.id] = (readingsByMeter[m.id] || []).length;
    }
    const maxReadings = Math.max(0, ...Object.values(meterReadingCounts));

    // Project-wide stats for outlier detection
    const allValues = [];
    ...
    const variance = allValues.length > 0
      ? allValues.reduce((acc, v) => acc + Math.pow(v - mean, 2), 0) / allValues.length
      : 0;
    ...
    for (const m of allMeters) {
      ...
      if (values.length === 0 || nonZeroValues.length === 0) {
        ...
        reason = values.length === 0
          ? 'No consumption readings found'
          : 'All readings are zero or empty';
      ...
      if (category) {
        anomalies.push({
          ...
          avg_consumption: values.length > 0
    ...
    // Sort: no_data → partial_data → need_investigation
    const order = { no_data: 0, partial_data: 1, need_investigation: 2 };
