...
- **Revoke is one click, no confirmation** — it's reversible, just flips the
  page's toggle off. **Delete is the one gated by typing the signer's name**,
  since removing the record and its PDF for good is the higher-stakes
  action of the two.
...
- Centered header, an addressee block ("To \<PROVIDER, in caps>" above "To
  whom it may concern,"), then the letter body.
- A divider near the bottom with **Confidential** centered underneath.
